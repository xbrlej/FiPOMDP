...
from fipomdp.pomcp_8_sparse_unshielded import OnlineStrategy
...
def log_experiment_with_seed(cpomdp, env, i, log_file_name, solver, targets):
    handler = logging.FileHandler(f"./logs_8_heavy_unshielded/{log_file_name}{i}.log", 'w')
...
def main():
    log_file_name = "UUVExperimentsFromPythonFile_8_heavy_unshielded"  # Change for your needs
    ...
    env = SingleAgentEnv(
        grid_size=[8, 8],
        capacity=12,
        reloads=[i*8+j for i in range(0, 8, 3) for j in range(0, 8, 3)],  # reloads in 5x5 grids
        targets=[14, 28, 34],
        init_state=63,
    ...
    with open("cpomdp_8_un", "wb") as cpomdp_file:

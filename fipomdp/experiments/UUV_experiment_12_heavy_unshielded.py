...
from fipomdp.pomcp_12_sparse_unshielded import OnlineStrategy
...
# SPECIFY ROLLOUT FUNCTION from rollout_functions.py
    ...
    #
    grid_adjusted = partial(grid_manhattan_distance, grid_size=[12, 12], targets=[46, 54, 87])
...
def log_experiment_with_seed(cpomdp, env, i, log_file_name, solver, targets):
    handler = logging.FileHandler(f"./logs_12_heavy_unshielded/{log_file_name}{i}.log", 'w')
...
def main():
    log_file_name = "UUVExperimentsFromPythonFile_12_heavy_unshielded"  # Change for your needs
    ...
    env = SingleAgentEnv(
        grid_size=[12, 12],
        capacity=16,
        reloads=[i*12+j for i in range(0, 12, 4) for j in range(0, 12, 4)],  # reloads in 5x5 grids
        targets=[46, 54, 75],
        init_state=143,
    ...
    cpomdp = mdp
    cpomdp.compute_guessing_cmdp_initial_state([143])

    with open("cpomdp_12_un", "wb") as cpomdp_file:
        pickle.dump(cpomdp, cpomdp_file)

    # with open("cpomdp_12", 'rb') as pickle_file:

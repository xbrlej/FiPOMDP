...
from fipomdp.pomcp_16_sparse_unshielded import OnlineStrategy
...
# SPECIFY ROLLOUT FUNCTION from rollout_functions.py
    ...
    #
    grid_adjusted = partial(grid_manhattan_distance, grid_size=[16, 16], targets=[74, 104, 132, 178])
...
def log_experiment_with_seed(cpomdp, env, i, log_file_name, solver, targets):
    handler = logging.FileHandler(f"./logs_16_heavy_unshielded/{log_file_name}{i}.log", 'w')
...
def main():
    log_file_name = "UUVExperimentsFromPythonFile_16_heavy_unshielded"  # Change for your needs
    ...
    env = SingleAgentEnv(
        grid_size=[16, 16],
        capacity=16,
        reloads=[i*16+j for i in range(0, 16, 4) for j in range(0, 16, 4)],  # reloads in 5x5 grids
        targets=[74, 104, 132, 178],
        init_state=255,
    ...
    cpomdp = mdp
    cpomdp.compute_guessing_cmdp_initial_state([255])

    with open("cpomdp_16_un", "wb") as cpomdp_file:
        pickle.dump(cpomdp, cpomdp_file)

    # with open("cpomdp_16", 'rb') as pickle_file:
    #    cpomdp = pickle.load(pickle_file)

    solver = ConsPOMDPBasicES(cpomdp, [255], env.capacities[0], targets)

...
from fipomdp.experimental.energy_solvers_unshielded import ConsPOMDPBasicES
...
class POMCPTree:
    ...
    def iteration(self, sampled_state: int) -> Tuple[POMCPHistoryNode, List[int]]:
        ...
        while current_history_node.visits != 0 and tmp_sample not in self.targets:
            ...
            tmp_sample = belief
# TODO: Why belief instead of node?
        self.iterations += 1
    ...
    def rollout(
        ...
        for i in range(horizon):
            ...
            if last_action is None:
                ...
                if random.random() <= 0.75*(0.99**i) and last_action in safe_actions:
...
class POMCPHistoryNode(POMCPNode):
    ...
    def simulate_action(self, sampled_state: int) -> Tuple[POMCPHistoryNode, int]:
        ...
        self.logger.info(f"BEST UCT: {max(uct_values)}, ALL UCT: {uct_values}")
...
class OnlineStrategy:
    ...
    def __init__(
        ...
        with open("as_16_un", "wb") as asfile:
            pickle.dump(action_shield, asfile)
        # with open("as_16", "rb") as asfile:
